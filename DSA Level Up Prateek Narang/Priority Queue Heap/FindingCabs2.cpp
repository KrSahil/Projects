...
void printNearestCars(vector<Car> cars,int k){
    ...
    priority_queue<Car, vector<Car>, carCompare> max_heap(cars.begin(),cars.begin()+k); //initializing heap with first K car 
    ...
    //sort(output.begin(),output.end()); //ther is one problem here, sort function doesn't know how to sort the car
    reverse(output.begin(),output.end());
    //reverse will work because if you extract car from the heap, they are going to be sorted in reverse order and we will not need any comparison operator

    //either return the vector or print it
    for(auto car: output){
        cout<<car.id<<endl;
    }
    return;
