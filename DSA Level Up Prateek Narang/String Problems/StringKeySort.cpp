...
string extractStringAtkey(string str, int key){
    //string tokeniser
    char *s = strtok((char*)str.c_str()," "); //did not understtand
    while (key>1){
        s = strtok(NULL," ");
        key--;
    }
...
int main(){
    ...
    vector<string> v;
    for(int i = 0; i<n; i++){
    //the function to get input whic contatins spaces is 
        getline(cin,temp); //includes white spaces
        v.push_back(temp);
    }
    //cin doesnt include white spaces
    //one entire line with white spaces - getline
    //one individiual input cin
    int key;
